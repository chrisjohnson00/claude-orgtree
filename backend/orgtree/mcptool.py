...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "now it is slipped in like any mail. In-org agents only — "
            "'user' and outside addresses (@org:/@mcp:/@net:) take "
            "orgtree_message, which is already passive for them. Use it for FYIs, progress notes and "
    ...
    },
    {
        ...
        "description": (
            ...
            "withdraw it (orgtree_withdraw_ask). If you hold no user "
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "status": {"type": "string",
                           "description": "create/update: backlogged|open|in_progress|blocked|review|dropped (done only via accept). `review` = REVIEW BY AGENTS, never how you ask the user for something (see identity_prompt's DOCKET_DOCTRINE for the full rules on blocked/dropped/waiting)"},
                ...
                "attention_reason": {"type": "string", "description": "update: the concrete reason the user must see. 'Ready for review' is not enough — see identity_prompt's DOCKET_DOCTRINE for what this field must contain"},
    ...
    },
    {
        ...
        "description": (
            ...
            "still matter afterwards.) The card is nulled and no answer will "
            ...
            "properties": {
                ...
                "deadline_minutes": {
                    ...
                    "description": "OPTIONAL, off by default. Force the "
                                   "deploy after this many quiet-free minutes "
                                   "(5–1440, requires `reason`) — stops "
    ...
    },
    {
        ...
        "description": (
            ...
            "again with `replaces` set to the returned id to update in place. "
            "HTML MOCKUPS: pass `path` (a .html/.htm file in your "
    ...
    },
    {
        ...
        "description": (
            ...
            "beside any question or credit tabs, and the user decides "
            "approve/deny/skip PER ITEM "
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "notice": {"type": "boolean",
                           "description": "create: fire passively instead "
                                          "of waking you (default false); "
                                          "see the top-level description"},
                "once": {"type": "boolean",
                         "description": "create: one-shot — fires once and "
                                        "removes itself (default false); "
                                        "see the top-level description"},
                "shell": {"type": "string", "enum": ["native", "bash"],
                          "description": "create, command/stream only: "
                                         "which shell interprets `target` — "
                                         "default \"native\", or \"bash\" for "
                                         "the POSIX idiom; see the top-level "
                                         "description"},
                "id": {"type": "string",
            ...
            "properties": {
                ...
                "hire_type": {
                    ...
                    "description":
                        ...
                        "so passing ANY of them is refused rather than "
                        "silently overwritten; retool it afterwards if it "
                        "should hold less. (In 'subordinate' mode the first "
                        "three are REQUIRED instead.) Costs the same as the "
                        "ordinary hire; the insertion itself moves no "
                ...
                "permission_mode": {
                    ...
                    "description":
                        ...
                        "working seat, 'plan' read-only. 'bypassPermissions' "
                        "is the only mode that can write a .claude path — "
                        "it removes guardrails on every path on the "
                        "machine, not just the one you had in mind. CAPPED "
                        "AT YOUR OWN."},
                "effort": {"type": "string",
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "prefer_reserve": {
                    "type": "boolean",
                    "description": "luna only — same reserve-capacity rule "
                                   "as orgtree_hire's prefer_reserve"},
            },
            "required": ["node"],
        },
    },
    ...
    },
    {
        ...
        "description": (
            "Cache-cheap reset of an idle report's session in place (seat id, "
            "parent, scope, charter, grant and team retained; prior session "
            "archived as a knowledge bearer <node>@<gen>, successor starts at "
            "zero context). Refused on yourself and on nodes with open "
            "background tasks."),
        "inputSchema": {"type": "object",
                        "properties": {"node": {"type": "string"}},
                        "required": ["node"]},
    },
    {
        "name": "orgtree_rehire",
        "description": (
            "Rehire an archived node in your subtree. Rehiring under an archived superior rehires the "
    ...
    },
    {
        ...
        "description": (
            ...
            "with the AGENT. Works for any pair, nested or disjoint. A swapped "
    ...
    },
    {
        ...
        "description": (
            ...
            "whole chain lacks it. Use orgtree_list_tiers for current offered "
            "ids, prices and advisory machine availability."),
        "inputSchema": {"type": "object",
                        "properties": {"node": {"type": "string"},
                                       "tier": {"type": "string",
